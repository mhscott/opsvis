    ...
    # elif nen == 20:
        ...
        for i, ele_tag in enumerate(ele_tags):
            ...
            ex = np.array([ops.nodeCoord(nd1)[0],
                           ...
                           ops.nodeCoord(nd13)[0],
                           ops.nodeCoord(nd14)[0],
                           ops.nodeCoord(nd15)[0],
                           ops.nodeCoord(nd16)[0],
                           ops.nodeCoord(nd17)[0],
                           ops.nodeCoord(nd18)[0],
                           ops.nodeCoord(nd19)[0],
                           ops.nodeCoord(nd20)[0]])
            ey = np.array([ops.nodeCoord(nd1)[1],
                           ops.nodeCoord(nd2)[1],
                           ops.nodeCoord(nd3)[1],
                           ops.nodeCoord(nd4)[1],
                           ...
                           ops.nodeCoord(nd13)[1],
                           ops.nodeCoord(nd14)[1],
                           ops.nodeCoord(nd15)[1],
                           ops.nodeCoord(nd16)[1],
                           ops.nodeCoord(nd17)[1],
                           ops.nodeCoord(nd18)[1],
                           ops.nodeCoord(nd19)[1],
                           ops.nodeCoord(nd20)[1]])
            ez = np.array([ops.nodeCoord(nd1)[2],
                           ops.nodeCoord(nd2)[2],
                           ops.nodeCoord(nd3)[2],
                           ops.nodeCoord(nd4)[2],
                           ...
                           ops.nodeCoord(nd8)[2],
                           ops.nodeCoord(nd9)[2],
                           ops.nodeCoord(nd10)[2],
                           ops.nodeCoord(nd11)[2],
                           ops.nodeCoord(nd12)[2],
                           ops.nodeCoord(nd13)[2],
                           ops.nodeCoord(nd14)[2],
                           ops.nodeCoord(nd15)[2],
                           ops.nodeCoord(nd16)[2],
                           ops.nodeCoord(nd17)[2],
                           ops.nodeCoord(nd18)[2],
                           ops.nodeCoord(nd19)[2],
                           ops.nodeCoord(nd20)[2]])
...
def _plot_defo_mode_2d(modeNo, sfac, nep, unDefoFlag, fmt_defo, fmt_undefo,
    ...
    if nen == 2:
        ...
        # beam/frame element plot_defo
            ...
            for ele_tag in ele_tags:
                ...
                if modeNo:
                    ed = np.array([*ops.nodeEigenvector(nd1, modeNo),
                                   *ops.nodeEigenvector(nd2, modeNo)])
                else:
                    ed = np.array([*ops.nodeDisp(nd1),
                                   *ops.nodeDisp(nd2)])
    ...
    # 2d triangular (tri31) elements plot_defo
        ...
        for ele_tag in ele_tags:
            ...
            if modeNo:
                ed = np.array([*ops.nodeEigenvector(nd1, modeNo),
                               *ops.nodeEigenvector(nd2, modeNo),
                               *ops.nodeEigenvector(nd3, modeNo)])
            else:
                ed = np.array([*ops.nodeDisp(nd1),
                               *ops.nodeDisp(nd2),
                               *ops.nodeDisp(nd3)])
        ...
        for ele_tag in ele_tags:
            ...
            if modeNo:
                ed = np.array([*ops.nodeEigenvector(nd1, modeNo),
                               *ops.nodeEigenvector(nd2, modeNo),
                               *ops.nodeEigenvector(nd3, modeNo),
                               *ops.nodeEigenvector(nd4, modeNo)])
            else:
                ed = np.array([*ops.nodeDisp(nd1),
                               *ops.nodeDisp(nd2),
                               *ops.nodeDisp(nd3),
        ...
        for ele_tag in ele_tags:
            ...
            if modeNo:
                ed = np.array([*ops.nodeEigenvector(nd1, modeNo),
                               ...
                               *ops.nodeEigenvector(nd3, modeNo),
                               *ops.nodeEigenvector(nd4, modeNo),
                               *ops.nodeEigenvector(nd5, modeNo),
                               *ops.nodeEigenvector(nd6, modeNo),
                               *ops.nodeEigenvector(nd7, modeNo),
                               *ops.nodeEigenvector(nd8, modeNo)])
            else:
                ed = np.array([*ops.nodeDisp(nd1),
                               *ops.nodeDisp(nd2),
                               *ops.nodeDisp(nd3),
        ...
        for ele_tag in ele_tags:
            ...
            if modeNo:
                ed = np.array([*ops.nodeEigenvector(nd1, modeNo),
                               ...
                               *ops.nodeEigenvector(nd7, modeNo),
                               *ops.nodeEigenvector(nd8, modeNo),
                               *ops.nodeEigenvector(nd9, modeNo)])
            else:
                ed = np.array([*ops.nodeDisp(nd1),
                               *ops.nodeDisp(nd2),
                               *ops.nodeDisp(nd3),
            ...
            if unDefoFlag:
                plt.plot(ex[[0, 4, 1, 5, 2, 6, 3, 7, 0]],
                         ey[[0, 4, 1, 5, 2, 6, 3, 7, 0]], fmt_undefo)
            ...
            plt.plot(x[[0, 4, 1, 5, 2, 6, 3, 7, 0]],
                     y[[0, 4, 1, 5, 2, 6, 3, 7, 0]], fmt_defo)
            plt.plot([x[8]], [y[8]], 'b.-')
    ...
    # 2d triangle (tri6n) elements plot_defo
        ...
        for ele_tag in ele_tags:
            ...
            plt.plot(x[[0, 3, 1, 4, 2, 5, 0]],
                     y[[0, 3, 1, 4, 2, 5, 0]], fmt_defo)
    ...
    # plot: quad in 3d defo, but also shell which case more dofs
        ...
        if ndf == 6:
            ...
            for i, ele_tag in enumerate(ele_tags):
                ...
                if modeNo:
                    ed = np.array([*ops.nodeEigenvector(nd1, modeNo)[:3],
                                   *ops.nodeEigenvector(nd2, modeNo)[:3],
                                   *ops.nodeEigenvector(nd3, modeNo)[:3],
                                   *ops.nodeEigenvector(nd4, modeNo)[:3]])
                else:
                    ed = np.array([*ops.nodeDisp(nd1)[:3],
                                   *ops.nodeDisp(nd2)[:3],
                                   *ops.nodeDisp(nd3)[:3],
                                   *ops.nodeDisp(nd4)[:3]])
        ...
        for i, ele_tag in enumerate(ele_tags):
            ...
            if modeNo:
                ed = np.array([*ops.nodeEigenvector(nd1, modeNo),
                               ...
                               *ops.nodeEigenvector(nd8, modeNo),
                               *ops.nodeEigenvector(nd9, modeNo),
                               *ops.nodeEigenvector(nd10, modeNo),
                               *ops.nodeEigenvector(nd11, modeNo),
                               *ops.nodeEigenvector(nd12, modeNo),
                               *ops.nodeEigenvector(nd13, modeNo),
                               *ops.nodeEigenvector(nd14, modeNo),
                               *ops.nodeEigenvector(nd15, modeNo),
                               *ops.nodeEigenvector(nd16, modeNo),
                               *ops.nodeEigenvector(nd17, modeNo),
                               *ops.nodeEigenvector(nd18, modeNo),
                               *ops.nodeEigenvector(nd19, modeNo),
                               *ops.nodeEigenvector(nd20, modeNo)])
            else:
                ed = np.array([*ops.nodeDisp(nd1),
                               *ops.nodeDisp(nd2),
                               *ops.nodeDisp(nd3),
            ...
            x = ex+sfac*ed[[0, 3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36, 39, 42, 45, 48, 51, 54, 57]]
            y = ey+sfac*ed[[1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34, 37, 40, 43, 46, 49, 52, 55, 58]]
            z = ez+sfac*ed[[2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35, 38, 41, 44, 47, 50, 53, 56, 59]]
            for j in range(2):
                plt.plot(x[[0+j*4, 8+j*4, 1+j*4, 9+j*4, 2+j*4, 10+j*4, 3+j*4, 11+j*4, 0+j*4]],
                         y[[0+j*4, 8+j*4, 1+j*4, 9+j*4, 2+j*4, 10+j*4, 3+j*4, 11+j*4, 0+j*4]],
                         z[[0+j*4, 8+j*4, 1+j*4, 9+j*4, 2+j*4, 10+j*4, 3+j*4, 11+j*4, 0+j*4]],
                         'b.-', lw=0.4, ms=2, mfc='g', mec='g')

            for j in range(4):
                plt.plot(x[[j, 16+j, 4+j]],
                         y[[j, 16+j, 4+j]],
                         z[[j, 16+j, 4+j]], 'b.-',
...
def plot_supports_and_loads_2d(nep=17, sfac=False):
    """Display the supports and the loads applied to the 2d beam-column models
    ...
    for ele_tag in ele_tags:
        ...
        for ele_load_data_i in ele_load_data:
            ...
            if ele_load_type == '-beamPoint':
                ...
                plt.text(sum(ex)/2, sum(ey)/2, f'{Pt}', color='b')
                # plt.annotate("",
                #              xy=(s_p[0], s_p[1]), xycoords='data',
                #              xytext=(s_0[0], s_0[1]), textcoords='data',
                ...
                plt.text(sum(ex)/2, sum(ey)/2, text_string, va='bottom', ha='center', color='r')
    ...
    for node_tag in node_tags:
        ...
        # display support symbols: fix - square, triangle - pin, circle - roller
        ...
        # alternative plot of supports - less readable
        # for i, node_dof in enumerate(node_dofs):
        #     if i == 0 and node_dof == -1:
        #         support_type, support_color = '-', 'm'
        #     if i == 1 and node_dof == -1:
        #         support_type, support_color = '|', 'm'
        #     if i == 2 and node_dof == -1:
        #         support_type, support_color = 'R', 'm'

        #     plt.text(nd_crd[0], nd_crd[1], support_type, color=support_color,
        #              va='center', ha='center', weight='bold', fontsize=32)
            ...
            for kier in nodal_loads_idx[0]:
                ...
                if kier == 0 or kier == 1:
                    ...
                    plt.arrow(nd_crd[0], nd_crd[1],
                              ...
                              fc='b', ec='b',
                              length_includes_head=True, shape='full')
                    plt.text(nd_crd[0]+dx, nd_crd[1]+dy, f'P = {nodal_loads[kier]}', color='b')
                ...
                # concentrated bending moment
                    ...
                    plt.plot(nd_crd[0], nd_crd[1], marker=marker_type, markersize=35, color='b')
                    plt.text(nd_crd[0], nd_crd[1], f'M = {nodal_loads[kier]}', color='b', va='bottom', ha='left')
...
def quad_crds_node_to_ip():
    ...
    for i, ele_tag in enumerate(ele_tags):
        ...
        eles_nds_crd[i] = np.array([[*ops.nodeCoord(nd1)],
                                    [*ops.nodeCoord(nd2)],
                                    [*ops.nodeCoord(nd3)],
                                    [*ops.nodeCoord(nd4)]])
...
def sig_out_per_ele_quad():
    ...
    for i, ele_tag in enumerate(ele_tags):
        ...
        vm_nd_out = extrapolate_ip_to_node_quad(vm_ip_out)
...
def sig_out_per_ele_brick():
    ...
    eles_ips_sig_out = np.zeros((n_eles, 8, 7))
    eles_nds_sig_out = np.zeros((n_eles, 8, 7))
    ...
    for i, ele_tag in enumerate(ele_tags):
        ...
        ind7 = node_tags.index(nd7)
        ind8 = node_tags.index(nd8)
        nodes_tag_count[[ind1, ind2, ind3, ind4, ind5, ind6, ind7, ind8], 1] += 1

        sig_ip_el = ops.eleResponse(ele_tag, 'stresses')
        sigM_ip = np.vstack(([sig_ip_el[0:6],
                              sig_ip_el[6:12],
                              sig_ip_el[12:18],
                              sig_ip_el[18:24],
                              sig_ip_el[24:30],
        ...
        # rearrange rows because stdBrick gauss points are ordered differently
        rows_order = np.array([0, 4, 6, 2, 1, 5, 7, 3])
        sigM_ip = sigM_ip[rows_order]

        sigM_nd = extrapolate_ip_to_node_brick(sigM_ip)
        eles_ips_sig_out[i, :, :6] = sigM_ip
        eles_nds_sig_out[i, :, :6] = sigM_nd

        vm_ip_out = vm_stress(np.transpose(eles_ips_sig_out[i, :, :6]))
        vm_nd_out = vm_stress(np.transpose(eles_nds_sig_out[i, :, :6]))
        eles_ips_sig_out[i, :, 6] = vm_ip_out
        eles_nds_sig_out[i, :, 6] = vm_nd_out
...
def plot_mesh_with_ips_2d(nds_crd, eles_ips_crd, eles_nds_crd, quads_conn,
                          eles_ips_sig_out, eles_nds_sig_out, sig_out_indx, lw=1.2):
    """
    ...
    plot_model(node_labels=0, element_labels=0, lw=lw)
    # plot_mesh_2d(nds_crd, quads_conn, lw=lw, ec='b')
